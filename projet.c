...
void playersname(Game* Game1){ // Procédure pour saisir les noms des joueurs

    for ( int i = 0 ; i < Game1->nb_player ; i ++ ){ // Parcours tous les joueurs
        printf("Saisir nom du joueur %d : ", i+1); // Demande à l'utilisateur de saisir le nom du joueur
        scanf("%s",Game1->player[i].name); // Lit le nom saisi par l'utilisateur et le stocke dans la structure du joueur correspondant
    }
...
int main() { // Fonction principale du programme
    ...
    system("chcp 65001"); 
    Game game1; // Créer une structure Game
    Game* pointer_game1 = &game1; // Déclarer un pointeur vers la structure Game
    playernumber(pointer_game1); // Demander et enregistrer le nombre de joueurs
    create_tab_player(pointer_game1); // Créer le tableau de joueurs
    create_dimension(pointer_game1);  // Définir les dimensions du plateau de jeu en fonction du nombre de joueurs
    create_tab_box(pointer_game1); // Créer le tableau du plateau de jeu
    verify_generer_poisson(pointer_game1); // Vérifier et générer les poissons sur le plateau
    put_penguin_on_tab(pointer_game1); // Placer les pingouins sur le tableau de jeu
    put_penguin_on_box(pointer_game1);  // Placer les pingouins sur le plateau  de jeu
    afficher_grille(pointer_game1); // Afficher la grille de jeu
    game_total(pointer_game1); // Lancer le jeu
