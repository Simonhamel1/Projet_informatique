...
char* give_caractere_penguin(int i, int j, Game* game1){ // fonction pour donner le caractère représentant un pingouin
    char* a = (char*)malloc(30 * sizeof(char)); // Allocation dynamique d'un espace mémoire pour stocker le caractère
    if (a == NULL) { // Vérification si l'allocation a échoué
    ...
    else { // Si un pingouin est présent à cette position
        char* color; // Déclaration d'une variable pour stocker la couleur
        switch (num_player) { // Switch basé sur le numéro du joueur
            case 1: color = RED; break; // Si le joueur est le joueur 1, couleur rouge
            case 2: color = YELLOW; break; // Si le joueur est le joueur 2, couleur jaune
            case 3: color = GREEN; break; // Si le joueur est le joueur 3, couleur verte
            case 4: color = BLUE; break; // Si le joueur est le joueur 4, couleur bleue
            case 5: color = CYAN; break; // Si le joueur est le joueur 5, couleur cyan
            case 6: color = WHITE; break; // Si le joueur est le joueur 6, couleur blanche
            default: // Cas par défaut (numéro de joueur non valide) 
                free(a); // Libération de la mémoire allouée précédemment
                exit(1); // Quitter le programme en cas d'erreur
        }
        
        strcpy(a, color); // Copie de la couleur dans la chaîne
        strcat(a, "🐧P"); // Ajout du symbole de pingouin à la chaîne
        strcat(a, give_num_penguin(num_player, i, j, game1)); // Ajout du numéro du pingouin à la chaîne
        strcat(a, RESET);  // Réinitialisation de la couleur après le pingouin
    }
        ...
        while(k<game1->nb_column){ // Boucle pour parcourir chaque colonne de la grille
            if(game1->box[a][k].existence==1){ // Vérifie si la case existe
                if(game1->box[a][k].penguin ==1){
                    printf(" %s %c", give_caractere_penguin(a, k, game1),92); // Affiche un pingouin dans une case
                }
                    ...
                    printf(" %s %c", give_caractere_penguin(a, k, game1),92); // Affiche une case vide ou bloquée
                }
            }
            else{
            ...
            if(m<game1->nb_column && (a!=(game1->nb_ligne-1) || m!=(game1->nb_column-1))){ // Vérifie s'il y a une colonne à afficher après la case
                if(game1->box[a][m].existence){ // Vérifie si la case existe
                    if(a!=(game1->nb_ligne-1) && game1->box[a][m].penguin==1){
                        printf(" %s %c", give_caractere_penguin(a, m, game1), 92); // Affiche un pingouin dans une case
                    }
